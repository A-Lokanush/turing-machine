...
void decimal_to_binary(string input, string& output) {
    int state = 0, i = 1; bool flag = false; output = "0";
        ...
        if (state == 0 && input[i] != '#') { i++; }
            ...
            if (input[i] == '0') {
                ...
                for (j = output.length() - 1; j >= 0; j--) {
                    if (output[j] == '0') { output[j] = '1'; break; } 
                    else if (output[j] == '1') { output[j] = '0'; }
                }
                if (j < 0) { output = "1" + output; }
            }
        } else if (state == 2) { break; }
    }
    if (flag) { cout << "ERROR: Invalid Input Detected \n"; } 
    else if (state != 2) { cout << "ERROR: Sorry Problem on our part\n";}
    else { cout << "\nBINARY : " << output << "\n"; }
}
